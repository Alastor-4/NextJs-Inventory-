...
import {
    AppBar, Box, Card, CardContent, Checkbox, Collapse,
...
const StoresMainTable = ({ userId, ownerId }: StoresMainTableProps) => {
    ...
    useEffect(() => {
        const getData = async () => {
            const newData = await stores.allUserStores(userId, ownerId!);
            setData(newData);
        }
        if (data === null) {
            getData();
    ...
    async function handleRemove() {
        ...
        if (response) {
            const updatedStores = await stores.allUserStores(userId, ownerId!);
    ...
    const handleUpdate = () => {
        router.push(`/inventory/keeper/store/update/${selected.id}`)
    }
    const handleCreate = () => {
        router.push(`/inventory/keeper/store/create`);
    }
                ...
                <Box sx={{ display: "flex" }}>
                    {
                        selected && (
                            <Box sx={{ display: "flex" }}>
                                <IconButton
                                    color={"inherit"}
                                    onClick={() => router.push(`/inventory/keeper/store-details/${selected.id}`)}
                                >
