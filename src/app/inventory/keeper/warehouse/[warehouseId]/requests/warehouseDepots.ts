...
const warehouseDepots = {
    ...
    depotStoreDistribution: async function (ownerId: number, warehouseId: number, depotId: number) {
        try {
            const response = await apiRequest.get(urlDepotAssign(warehouseId), { params: { ownerId: ownerId, depotId: depotId } })
            return response.data
        } catch (e) {
            //ToDo: notify error here
        }
